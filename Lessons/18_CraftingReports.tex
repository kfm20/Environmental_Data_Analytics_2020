...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#command, option, i to open an R chunk}
\end{Highlighting}
\end{Shaded}
...
\item
  \emph{italics}
\item
  \textbf{bold} \emph{Helps to organize and annotate code so others can
  understand what the code is saying }visually pleasing when knit the
  document to a pdf \emph{reproducible }SEtting up in chunks allows you
  to control which sets of code are being run *puts code, text,
  graphs/figures/test results all in one document
\end{itemize}

*\#use these to make lists \#\# TEXT EDITING CHALLENGE headers with 2
hashtags, and subheadings with 3 hashtag show up in outline in top right
of rmd
...
\begin{longtable}[]{@{}ll@{}}
\toprule
Dataset Name & Information\tabularnewline
\midrule
\endhead
LTER & Water nutrient, and chemistry data on Peter and Paul
Lake\tabularnewline
NEON\_NIWO & Litter mass\tabularnewline
EPA Air & Ambient Air Quality\tabularnewline
USGS & Flow data\tabularnewline
\bottomrule
\end{longtable}
