...
```{r}
#command, option, i to open an R chunk
```
...
* *italics*
* **bold** 
*Helps to organize and annotate code so others can understand what the code is saying
*visually pleasing when knit the document to a pdf
*reproducible 
*SEtting up in chunks allows you to control which sets of code are being run
*puts code, text, graphs/figures/test results all in one document

*#use these to make lists
## TEXT EDITING CHALLENGE
headers with 2 hashtags, and subheadings with 3 hashtag show up in outline in top right of rmd
...
-------------|-----------------
LTER |Water nutrient, and chemistry data on Peter and Paul Lake
NEON_NIWO | Litter mass
EPA Air    |Ambient Air Quality
USGS |Flow data
...
LakeNutrients<-read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(LakeNutrients)
LakeNutrients$sampledate<-as.Date(LakeNutrients$sampledate, format ="%m/%d/%y")
```
...
```{r Processed Dataset}
LakeNutrients_Processed<- LakeNutrients %>% 
  select(lakename:sampledate, depth:po4) %>% filter(depth== 0)
