...
ui <- fluidPage(theme = shinytheme("superhero"),
      ...
      checkboxGroupInput(inputId = "fill",
                         label = "Depth ID",
                         choices = unique(nutrient_data$depth_id), #unique will duplicate replicates
                         selected = c(1, 7)),
    ...
    # Define reactive formatting for filtering within columns
     filtered_nutrient_data <- reactive({
        nutrient_data %>%
         ...
         filter(sampledate >= input$x[1] & sampledate <= input$x[2]) %>% #1 and 2 are specified as min and max
    ...
    # Create a ggplot object for the type of plot you have defined in the UI  
       output$scatterplot <- renderPlot({
        ggplot(filtered_nutrient_data(), #dataset
          ...
          labs(x = "Date", y = "Nutrient Concentration", shape = "Lake", fill = "Depth ID") +
