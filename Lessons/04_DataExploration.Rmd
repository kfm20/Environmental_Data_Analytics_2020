...
#This is specific to Kateri's computer
#Can also set it by Session on top tab> set working directory
```
...
```{r, message = FALSE}  
# Load package #R has a hard time knitting the check marks after running the loading of package, but the message=FALSE above tells it to not print this when knitted. Put this when loading tidyverse
library(tidyverse)
```
...
# Relative file path (friendly for users regardless of machine) start with dot which means go one fodler in      two dots mean go out one folder
#This is the PREFERABLE way to upload, because it is reproducible
...
# Another option is to choose with your browser, navigate to the file, but not reproducible
...
> ANSWER: This file is found in the metadata folder, which clearly distinguishes what this document is. It also has a similar name to the actually dataset file as well, which helps them be easily connected. It very specifically describes what each part of the data set means, where it was found, and by whom it was assembles.It tells you when the dataset was accessed which helps youd etermine if there were updates.
...
#Changed data frame within R, but need to code it to actually save these changes in the dataframe, will later rename and code it to save the new version
str(USGS.flow.data)
dim(USGS.flow.data)
length(USGS.flow.data) #Gives the number of columns, basically referring to width
#nrow(USGS.flow.data) will give height

head(USGS.flow.data) #default gives first 6 rows
head(USGS.flow.data, 10) #10 specifies how many rows you want
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #rows, columns

class(USGS.flow.data$datetime) 
#Asks for specific columns
...
> Answer: They are now noted as NA
Some have NA and some have blanks. Ones with NA are numeric, and ones that are blank are defined as character/factor level
...
> Answer: 
NA's screw up correlation matrix

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #rename it so you can go back to old dataframe without the changes you are going to make
#We will learn how to omit for certain columns
dim(USGS.flow.data)
dim(USGS.flow.data.complete) #eliminated all the rows that had an NA, regardless of how many other data points are in the row
...
### Formatting dates
**May look like a date but it is not intuitively known by R as a date**
...
#year, monthm day is natural to many
format(today, format = "%a")
format(today, format = "%Y")
#format(today, format = "")

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") 
#assigning the column in the file as a date, and **tells what format that exists in the data RIGHT NOW
```
