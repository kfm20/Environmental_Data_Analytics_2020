...
###Just generates file path and spits it out in text, doesn;t actually do anything with it
...
#reading in a shape file-- st.read different than read.csv
...
It already has a coordinate reference system, I know it has it, but telling R it has it
...
If coordinate system  exists but changing it into a new coordinate system
...
#na_albers_proj4 <- 
#na_albers_epsg <- 
# set the proj4 string fist and then set the EPSG code (because proj4 will over-ride EPSG if you set EPSG first)
...
# add your code here
Virginia<- southeast_state_bounds %>% filter(NAME== "Virginia")%>%
...
Virginia_basins_nf_seplains<-basins_nf_seplains%>%
  st_intersection(Virginia) #intersection same as clip in ARCGIS
# check
# add your code here
head(Virginia_basins_nf_seplains)

# select gages that fall within NC bounds
# add your code here
virginia_gages<- gages %>%
  st_intersection(Virginia)
# bonus
# add your code here
virginia_area<-st_area(Virginia) #area of VA
virginia_area

va_centroid<-st_centroid(Virginia)#find centroid of VA
va_centroid

vabuffer<-st_buffer(virginia_gages, dist=100) #buffer around stream gages in VA

not_va_gages<- st_difference(gages, Virginia) #stream gages not in VA
```
...
```{r basic.sf.plotting}
# basic plot
pdf(here("outputs", "southeast_state_bounds_today.pdf"), width = 11, height = 8.5)
ggplot() + #put in pdf because faster
  geom_sf(data = southeast_state_bounds) + #serves as bottom layer
  #geom_sf(data=, aes()) this would be the next layer on top
  # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines
  theme_bw() # add this to make the background white and not grey
dev.off()

# adding labels
pdf(here("outputs", "southeast_state_bounds_with_labs_today.pdf"), width = 11, height = 8.5)
...
pdf(here("outputs", "southeast_state_bounds_by_area_today.pdf"), width = 11, height = 8.5)
...
#lets look at just VA
pdf(here("outputs", "Virginia"), width = 11, height = 8.5)
ggplot() + 
...
pdf(here("outputs", "spatial_operations_activity_today.pdf"), width = 11, height = 8.5)
# add your code here
ggplot()+
  geom_sf(data= Virginia)+
  geom_sf(data=Virginia_basins_nf_seplains, fill="Pink", alpha=0.25) + 
  #alpha is transparency, showing the overlapping watrsheds
  geom_sf(data= virginia_gages, color="black" ) +
  theme_bw()
dev.off()


#dropping attributes and simply keeping geometry allows the plots to load faster
```
...
nc_bbox <- nc_state_bounds_geom %>%
  st_buffer(dist = 150000) %>% # this value is very arbitrary just wanting to make a large buffer around geometry, showing more than just NC, 150,000m
  st_transform(4326) %>% # WGS84 (for lat and long)
  #projecting
  #changing back to lat/long, it was in albersequal area
  st_bbox() #finds bounding box edges coordinate to create an actual box

nc_bbox # check- gives coordinates of the box
...
#using outputs from ncbbox to create box
...
***mutate(my_tabular_data_raw) ##need to make them both the same type, either factor or numeric 

# join the tabular data to nc_gages (spatial)
#allowing us to use the attrubutal data to color the different gage points by
