...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #Themes, specific to background
print(O3plot1)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #white background with black outline
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() #white background, gray outlines
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #full white background with no grid, black outline of plot
print(O3plot4)
```
...
#top or right legend position are frequently used
...
#call up a theme like above or set the theme like below
...
#great help function for theme or google on stackoverflow
```
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  #geom_hline is horizontal line, need to tell intercept, lty is line type, 2=dash
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) +
  #add points after line because ggplot does things in layers, so put in order
  ...
  #geom_text need to specify x,y coordinates of where to put it, what to call it, horiz or vert adjustment and which side-R or L
  #labeling particular areas of the data
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +
  #making certain date range on x axis, adjust breaks and tell what you want the labels to be-three letter month, and 2 number year
  ylab(expression("O"[3]* " AQI Value")) +
  #naming y axis label, brackets with star does small 3
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))

#adding adjustment to my set theme, specifying angle and hjust of x axis labels
...
NvsP2 <-
  ...
  labs(x=expression(paste("TP(", mu, "g/L)")), 
       ...
       color= "Depth (m)", shape = "")+ 
  #color referring to legend because we specified the color to be distinguished as the depths
...
# place your additional edits here
  labs(y=expression(paste("Concentration(", mu, "g/L)")), x="", color= "Nutrients") +
  scale_y_continuous(expand = c(0, 0)) +
  #scale_color_brewer(palette = "YlGnBu") +
  #scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84")) +
  scale_color_viridis(discrete = TRUE, end = 0.8, labels = c("NH34", "NO23", "PO4", "TN UG", "TP UG")) +
  theme(legend.position = "right")
print(Nutrientplot)
```
