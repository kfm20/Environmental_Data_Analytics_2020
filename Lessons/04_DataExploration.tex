...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(USGS.flow.data, }\DecValTok{10}\NormalTok{) }\CommentTok{#10 specifies how many rows you want}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#Asks for specific columns}
\KeywordTok{head}\NormalTok{(USGS.flow.data}\OperatorTok{$}\NormalTok{datetime)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#year, monthm day is natural to many}
...
\CommentTok{#assigning the column in the file as a date, and **tells what format that exists in the data RIGHT NOW}
\end{Highlighting}
\end{Shaded}
