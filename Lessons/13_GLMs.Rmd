...
>Categorical and continuous in the same
...
#Results: intercept represents Paul lake at zero depth



# interaction effects
#might have different intercepts and slopes for both lakes
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)
#intercept as Paul lake at surface depth
...
TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) +
  ...
  xlim(0, 10)
print(TNplot)
#show sus there is a significantly different slope between the two lakes. 
#Paul lake TN increasing as go deeper, Peter Lake, TN relatively consistently/slightly decreasing as go deeper
...
#Visualize the data before selecting a model
NTLtemps <-
...
TempTest.mixed <- lme(data = NTL.summertemp,
                     temperature_C ~ year4, 
                     random = ~1|lakename) #using the line like in pipe that means or, but here its saying we are grouping by lakename
...
#graphing the fixed effects results, 
NTL.tempmodel <-
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d() + #line of best fit representing the fixed effect model
  geom_abline(intercept = -97.72, slope = 0.06) + #mixed which had lower AIC
  # make it look better
  labs(x = "", y = expression("Temperature " ( degree~C)), color = "") +
  theme(legend.spacing.x = unit(0, "cm"))
print(NTL.tempmodel)

#we selected the mixed effect model because AIC lower, lake name now becomes a random effect
#Mixed effects model, lme R for resources to learn 
```

Question: How would you interpret the collective results of your mixed effects model in the context of the study question?

> ANSWER: Lake temperature is increasing over time across all of the lakes. If removed one of the lakes, it wouldn;t make a difference, they are all experiencing the same trend. 