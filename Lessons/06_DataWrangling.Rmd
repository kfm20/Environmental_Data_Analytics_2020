...
> Add notes here: Show seasonal changes, important for algal bloom documentation/primary productivity, large scale climatic changes in the region, potential links to food web changes
...
head(NTL.phys.data) #first 6 rows of each of columns in data frame
summary(NTL.phys.data)
str(NTL.phys.data) #gives what type of data,
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date, call up the column and what format it is in right now, R then puts it in year-month-day
...
```{r, results = "hide"}
vignette("dplyr")
#more useful than basic help function
#hide above in code chunk to avoid whole vignetter showing up when knitting
```
...
class(NTL.phys.data$depth) #noted in metadata, but depth is in meters

# matrix filtering- coma after the zero is saying the rows where depth equals zero and all the columns
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering, doing same thing as above
...
summary(NTL.phys.data$lakename) #gives # of cells the data represents
...
#Include anything that is Paul ad Peter Lake, using %in% we concatinate and use a coma instead of the statements made above
...
summary(NTL.phys.data$daynum) #samples from 55th day of year to 307th, not sampling during frozen lake

#Filtering data set for certain time periods
#comas can be same as & function here 
...
NTL.phys.data.19901999.TuesdayLake <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
dim(NTL.phys.data.19901999.TuesdayLake)
```
...
#summer at shallow depths having higher temperature
#NA's will show up at the bottom of the data
```
### Select
...
#selecting certain columns based on their names, filter is always horizontal and selecting/rganizing based on rows
#simply use commas to select
#to say you don;t want it to include something add a minus before the column name
#the line would all have to include, all exclude certain columns
...
```{r}
#adds column to the end of your data frame
#Mutating the specific data frame, calling it X, and use column of temp C and calculate 

NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32)
 # To rearrange columns, use the select to call them out in the specific order you want them listed in
```
...
```{r}
#pipe operator %>%, meaning "then", allowing all operations to happen within single span of code
